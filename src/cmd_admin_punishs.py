...
async def ban_member(message: types.Message, command: CommandObject):
    ...
    Аргументы:
    :time - время в дня, на которое банится пользователь
    """
    ...
    is_vip = get_member_chat_info(message.chat.id, message.reply_to_message.from_user.id)
...
async def warn_member(message: types.Message):
    ...
    is_exist = is_user_exists_chat_db(message.chat.id, message.reply_to_message.from_user.id)
    if is_exist == False: 
        add_user(message.chat.id, (message.reply_to_message.from_user.id, message.reply_to_message.from_user.username, 0, message.reply_to_message.from_user.first_name, 1, 0, 0, 0, 20, 0))
    ...
    warns = change_warns(message.chat.id, message.reply_to_message.from_user.id, '+')
...
async def unwarn_member(message: types.Message):
    ...
    total_warns = get_member_chat_info(message.chat.id, message.reply_to_message.from_user.id)
    ...
    total_warns = change_warns(message.chat.id, message.reply_to_message.from_user.id, '-')
