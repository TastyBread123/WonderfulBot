...
async def start(message: types.Message):
    ...
    if is_user_exists_chat_db(message.chat.id, message.from_user.id) is False:
        result = add_user(message.chat.id, (message.from_user.id, message.from_user.username, 0, message.from_user.first_name, 0, 0, 0, 0, 20, 0))
...
async def startbot(message: types.Message):
    ...
    data = get_config_data(message.chat.id)     
    ...
    create_chat_in_db(message.chat.id, start_info=start_info)
