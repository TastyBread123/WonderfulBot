...
async def reg_member(message: types.Message):
    ...
    get_admin = get_admin_lvl(message.chat.id, message.from_user.id)
    ...
    data = is_user_exists_chat_db(message.chat.id, message.reply_to_message.from_user.id)
    if data == False:
        ...
        add_user(message.chat.id, user_info)
