...
async def random_chisl(message):
    try:
        ...
        await message.reply("⚠️ Ошибка радиуса или в аргументах используется текст!\n\nИспользуйте: */rand <ОТ> <ДО>*", parse_mode='Markdown')
        ...
        if message.reply_to_message is None:
            ...
            try:
                ...
                if int(give_vip) == 1:
                    ...
                    await message.reply('⚠️ *Неверный аргумент! 1 - выдать/2 - забрать!*', parse_mode='Markdown')
...
@dp.message_handler(commands=['rankk'])
async def rankk(message):
    user_id = message.from_user.id

    get_ava = await bot.get_user_profile_photos(user_id)

    card = Image.open(f'{dir}imgs/ranking-card.png')
    ava = get_ava['photos']

    for photo_sizes in ava:
        photo = max(photo_sizes, key=lambda x: x['width'])
        file_id = photo['file_id']
        await message.answer_photo(ava, caption=file_id)
