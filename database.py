...
async def create_chat_in_db(chat_id: int | str, start_info: tuple):
    ...
    async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
        await groups_db.execute(f"CREATE TABLE IF NOT EXISTS chat_{get_info.get_chat_db_id(chat_id)}(id INT, login TEXT, admin INT, nick TEXT, warns INT, vip INT, total_exp INT, tolvl_exp INT, need_exp INT, level INT)")
        await groups_db.execute('CREATE TABLE IF NOT EXISTS config(id INT, welcome TEXT, rules TEXT)')
        await groups_db.execute(f"INSERT INTO chat_{get_info.get_chat_db_id(chat_id)} (id, login, admin, nick, warns, vip, total_exp, tolvl_exp, need_exp, level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (start_info))
...
async def add_user(chat_id: int | str, user_info: tuple):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            await groups_db.execute(f"INSERT INTO chat_{get_info.get_chat_db_id(chat_id)}(id, login, admin, nick, warns, vip, total_exp, tolvl_exp, need_exp, level) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (user_info))
            await groups_db.commit()
...
async def is_user_exists_chat_db(chat_id: int | str, user_id: int | str) -> bool:
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            is_exist = await groups_db.execute(f"SELECT id FROM chat_{get_info.get_chat_db_id(chat_id)} WHERE id = ?", (user_id,))
...
async def get_config_data(chat_id: int | str):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            data = await groups_db.execute(f"SELECT rules, welcome FROM config WHERE id = '{get_info.get_chat_db_id(chat_id)}'")
...
async def set_config_data(chat_id: int | str, column: str, data: int | str):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            await groups_db.execute(f"UPDATE config SET {column} = ? WHERE id = '{get_info.get_chat_db_id(chat_id)}'", (data,))
            await groups_db.commit()
...
async def get_member_chat_info(chat_id: int | str, user_id: int | str):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            data = await groups_db.execute(f"SELECT id, login, admin, nick, warns, vip, total_exp, tolvl_exp, need_exp, level FROM chat_{get_info.get_chat_db_id(chat_id)} WHERE id = ?", (user_id,))
            data = await data.fetchone()
...
async def set_member_chat_info(chat_id: int | str, user_id: int | str, column: str, data):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            await groups_db.execute(f"UPDATE chat_{get_info.get_chat_db_id(chat_id)} SET {column} = ? WHERE id = ?", (data, user_id))
            await groups_db.commit()
...
async def get_admin_lvl(chat_id: int | str, user_id: int | str):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            lvl = await groups_db.execute(f"SELECT admin FROM chat_{get_info.get_chat_db_id(chat_id)} WHERE id = ?", (user_id,))
...
async def add_message_chat_user(chat_id: int | str, user_id: int | str):
    ...
    try:
        async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
            await groups_db.execute(f"UPDATE chat_{get_info.get_chat_db_id(chat_id)} SET total_exp = total_exp + {1}, tolvl_exp = tolvl_exp + {1} WHERE id = ?", (user_id,))
            await groups_db.commit()
            messages_check = await groups_db.execute(f"SELECT tolvl_exp, need_exp FROM chat_{get_info.get_chat_db_id(chat_id)} WHERE id = ?", (user_id,))
            messages_check = await messages_check.fetchone()
            
            if messages_check[0] >= messages_check[1]:
                await groups_db.execute(f"UPDATE chat_{get_info.get_chat_db_id(chat_id)} SET level = level + {1}, tolvl_exp = {0}, need_exp = need_exp + {100} WHERE id = ?", (user_id,))
                await groups_db.commit()
                current_level = await groups_db.execute(f"SELECT level FROM chat_{get_info.get_chat_db_id(chat_id)} WHERE id = ?", (user_id,))
...
async def change_warns(chat_id: int | str, user_id: int | str, action: str):
    ...
    async with aiosqlite.connect('groups.db', check_same_thread=False) as groups_db:
        ...
        current_level = await groups_db.execute(f"SELECT warns FROM chat_{get_info.get_chat_db_id(chat_id)} WHERE id = ?", (user_id,))
        current_level = await current_level.fetchone()[0]
